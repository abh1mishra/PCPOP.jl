function mons_at_levelint(list_vars::Vector{Variable}, level::Int)
    ...
    for _ in 2:level
        temp_=union(
            monomials.(
                filter(res->(res!=Id && !(typeof(res)<:Number)), kron(temp_, list_vars, 1)),
            )...,
        )
        union!(mons, temp_)
    end
...
function mons_at_level(list_vars::Vector{Variable}, level::String)
    ...
    for i in total_types
        ...
        if (typeof(list_vars[1].parent_monoid[]))<:GraphProductMonoid
            if length(na_nu)==1
                type_var_dict[i]=filter(
                    x->extract_string_before_number(x.name)==i,
                    list_vars,
                )
            else
                rng_arr=parse_range(na_nu[2][1:(end - 1)])
                type_var_dict[i]=filter(
                    x->extract_string_before_number(x.name)==na_nu[1] &&
                       extract_index(x.name) in rng_arr,
                    list_vars,
                )
            end
        else
...
function get_monomials(
    ...
    if !isempty(list_vars)
        ...
        pols=vcat(
            [obj, op_ge..., op_eq...],
            [tr_ge[i][1] for i in 1:length(tr_ge)],
            [tr_eq[i][1] for i in 1:length(tr_eq)],
        )
    ...
    extra_vars = unique_array(
        union([[variables(g) for g in op_ge]; [variables(g) for g in op_eq]]...),
    )
    ops_add = mons_at_level(extra_vars, deg)
    ops_principal =
        unique_array([ops_add[o]*ops[p] for o in 1:length(ops_add) for p in 1:length(ops)])
...
function basis_gen(obj, level, op_eq, op_ge, tr_eq, tr_ge, list_vars, lvl_lm)
    ...
    if all(
        ...
        isempty(list_vars) && throw(
            ArgumentError(
                "The list of variables is empty. Please provide a non-empty list of variables.",
            ),
        )
    end
    # at this pont, op_ge and op_eq constaints non-trivial polynomials or zero.
    if lvl_lm==-1
        ops, ops_principal = get_monomials(
            obj,
            level;
            op_eq = op_eq,
            op_ge = op_ge,
