...
struct NCMonoid <: AbstractMonoid
    ...
    abs_alg_vars::Vector{AbstractAlgebra.Generic.FreeAssociativeAlgebraElem{Rational{BigInt}}}
    var_dict::Dict{Variable,AbstractAlgebra.Generic.FreeAssociativeAlgebraElem{Rational{BigInt}}}
    rev_var_dict::Dict{AbstractAlgebra.Generic.FreeAssociativeAlgebraElem{Rational{BigInt}},Variable}
    relations::Array{AbstractAlgebra.Generic.FreeAssociativeAlgebraElem{Rational{BigInt}}, 1}
...
function NCMonoid(name::String,variables::Vector{V};
    parent_monoid::Base.RefValue{AbstractMonoid}=Base.RefValue{AbstractMonoid}()) where V <: Variable
    ...
    ncmonoid=NCMonoid(name,parent_monoid,Base.RefValue(false),base_ring,variables,vars,var_dict,rev_var_dict,
...
struct NCWord <: AbstractMonomial
    ...
    function NCWord(monoid::NCMonoid,word::AbstractAlgebra.Generic.FreeAssociativeAlgebraElem{Rational{BigInt}})
        new(monoid,word,Base.RefValue{AbstractMonomial}())
    end
    function NCWord(monoid::NCMonoid,word::AbstractAlgebra.Generic.FreeAssociativeAlgebraElem{Rational{BigInt}},m::Base.RefValue{AbstractMonomial})
        new(monoid,word,m)
    end
end
...
function Base.show(io::IO, mime::MIME"text/plain",monoid::NCMonoid)
    ...
    print(io,"The Non-Commutative MONOID IS $(monoid.is_built[] ? "BUILT" : "NOT YET BUILT")\n 
...
function Base.isless(w1::NCWord,w2::NCWord)
    (w1.monoid!=w2.monoid) && w1.monoid.parent_monoid[]!=w2.monoid.parent_monoid[] && throw(ArgumentError("NOT YET IMPLEMENTED"))
    if (w1.monoid==w2.monoid) 
        w1.word<w2.word
    else
        vertices=w1.monoid.parent_monoid[].vertices
        return findfirst(item -> item == w1.monoid, vertices) < findfirst(item -> item == w2.monoid, vertices)
    end
end
...
function Base.conj(w::NCWord)
    ...
    for (index,coeff) in zip(1:length(w_),collect(coefficients(w_)))
        ...
        res_word+=coeff'*prod([var_dict[conj(vars[value])] for value in rev_expo_word],init=1)
    end
...
function multiply_ncword(w1::NCWord,w2::NCWord)
    ...
    res=(monoid.is_built[] && !isempty(replacement_rules)) ? normal_form(w1.word*w2.word,replacement_rules) : w1.word*w2.word
...
function Base.isless(w1::NCWord,w2::GraphProductWord)
    w1==w2 && return false
    w1.monoid.parent_monoid[]!=w2.monoid.parent_monoid[] && throw(ArgumentError("NOT YET IMPLEMENTED"))
    vertices==w1.monoid.parent_monoid[].vertices
    return findfirst(item -> item == w1.monoid, vertices) < findfirst(item -> item == w2.monoid, vertices)
end
