...
Base.show(io::IO, mime::MIME"text/plain", monoid::Tuple{Vararg{Monoid}}) where Monoid<:AbstractMonoid = print(io,Tuple((m.name for m in monoid)))
Base.show(io::IO, mime::MIME"text/print", monoid::Tuple{Vararg{Monoid}}) where Monoid<:AbstractMonoid = print(io,Tuple((m.name for m in monoid)))
