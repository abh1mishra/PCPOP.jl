...
function words_to_monomial(
    monoid::GraphProductMonoid{Variable},
    words::Vector{Variable};
    edge_l = Set{UInt32}(),
    edge_r = Set{UInt32}(),
)
    ...
    if isempty(edge_l)
        union!(
            monomial.edge_l,
            get_edge_indices(
                monomial.clique_words,
                :first,
                collect(1:length(monoid.cliques)),
                monoid,
            ),
        )
    else
...
function words_to_monomial(
    monoid::GraphProductMonoid{T},
    words::Vector{AW};
    edge_l = Set{AW}(),
    edge_r = Set{AW}(),
) where {T <: AbstractMonoid, AW <: AbstractMonomial}
    monomial = one(monoid)
    words=filter(i->!is_identity(i), words)
    isempty(words) && return monomial
    # for single variable
    if length(words) == 1
        word = words[1]
        clique_indices=isa(word, Variable) ? word.clique_indices :
                       word.monoid.clique_indices
    ...
    union!(
        monomial.edge_r,
        get_edge_variables(monomial.clique_words, :last, collect(1:length(monoid.cliques))),
    ) : union!(monomial.edge_r, edge_r)
    return monomial
end
...
function monomial_to_graph(monomial::GraphProductWord)
    ...
    non_commuting_pairs =
        [(node[j], node[j + 1]) for node in clique_word_node for j in 1:(length(node) - 1)]
...
function generate_labels(exponents::Dict)
    ...
    node_dict =
        eltype(keys(exponents))<:Variable ? Dict{Tuple{Variable, Int64}, Int64}() :
        Dict{Tuple{AbstractMonomial, Int64}, Int64}()
...
function label_clique_words(clique_words::Vector, cliques::Vector, node_dict::Dict)
    ...
    for (clique, word) in zip(cliques, clique_words)
        ...
        for monoid in clique
            ...
            variable_indices =
                eltype(eltype(cliques)) <: Variable ? findall(x -> x==monoid, word) :
                findall(x -> x.monoid==monoid, word)
            num_variable_occurrences = length(variable_indices)
            if (num_variable_occurrences != 0)
                [
                    labelled_word[index] = node_dict[(word[index], occurrence)] for
                    (index, occurrence) in zip(variable_indices, 1:num_variable_occurrences)
                ]
            end
        end
...
"""
function clique_words_to_labelled_word(
    node_dict::Dict,
    exponents,
    monomial::GraphProductWord,
)
    labelled_word = Vector{Int}()
    labelled_clique_words =
        label_clique_words(monomial.clique_words, monomial.monoid.cliques, node_dict)
...
function graph_to_foata(graph::SimpleDiGraph, node_dict::Dict)
    ...
    for (node, index) in node_dict
        if !(index in source_nodes)
            distance = maximum([
                length(a_star(graph, source_node, index)) for source_node in source_nodes
            ])
            (haskey(distance_dict, distance)) ? push!(distance_dict[distance], node) :
            distance_dict[distance] = [node]
        end
    end
    levels = length(distance_dict)
    # foata_form = Vector{GraphProductWord}(undef, levels)
    foata_form =
        Vector{Vector{Base.tuple_type_head(eltype(keys(node_dict)))}}(undef, levels)
    ...
    for i in 0:(levels - 1)
        foata_form[levels - i] =
            length(distance_dict[i]) == 1 ? [distance_dict[i][1][1]] :
            sort([node[1] for node in distance_dict[i]])
    end
    return foata_form
end
...
function monomial_to_graph(monomial::PCMonomial)
    ...
    node_dict =
        Dict((var_to_index(var), occ) => label for ((var, occ), label) in node_dict_var)
