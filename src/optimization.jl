...
function sos(
    ...
    matrix_psd = Dict(
        s => [tracial_reduce(x'*s*y, tracial = tracial) for x in basis, y in basis] for
        s in cores
    )
    ...
    for (idx, b) in enumerate(basis_constraints)
        if typeof(b) <: AbstractMonomial
            ...
            JuMP.@constraint model sum(
                cs*LinearAlgebra.dot(P[s], Γ[ms] .== idx) for s in cores_psd for
                (ms, cs) in s
            ) == c
...
function sos_block_diag(
    ...
    Γ = Dict(s => [(basis_constraints[m]) for m in matrix_psd[s]] for s in cores)
    basis_psd = Dict(
        s => StarAlgebras.Basis{UInt16}(union(sum(c*matrix_psd[m] for (m, c) in s))) for
        s in cores_psd
    )
    ...
    for s in cores_psd
        ...
        blkD = SDPSymmetryReduction.blockDiagonalize(
            Ps,
            verbose,
            epsilon = epsilon,
            complex = complex,
    ...
    P = Dict(
        s => [JuMP.@variable(model, [1:i, 1:i] in PSDCone()) for i in blocks[s].blkSizes] for s in cores_psd
    )
...
function moments_block_diag(
    ...
    # PSD constraints
    for s in cores_psd
        basis_psd = StarAlgebras.Basis{UInt16}(union(sum(c*matrix_psd[m] for (m, c) in s)))
        ...
        psdBlocks = sum(
            blkD.blks[i] .* sum(ci*y[basis_constraints[mi]]) for
            (mi, ci) in basis_psd[i] for i in 1:P.nparts
        )
    ...
    for s in cores_zero
        JuMP.@constraint model sum(
            c*y[x' * m * y] for (m, c) in s for x in basis for y in basis
        ) .== 0
    end
