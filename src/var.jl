...
function Base.isless(a::Variable,b::Variable)
    (a.parent_monoid[]!=b.parent_monoid[]) && a.parent_monoid[].parent_monoid[]!=b.parent_monoid[].parent_monoid[] && throw(ArgumentError("NOT YET IMPLEMENTED"))
    if(a.parent_monoid[]!=b.parent_monoid[])
        vertices=a.parent_monoid[].parent_monoid[].vertices
        return findfirst(item -> item == a.parent_monoid[], vertices) < findfirst(item -> item == b.parent_monoid[], vertices)
    end
...
macro ncmonoid(M,args...)
    ...
    push!(exprs,:($(esc(M))=$NCMonoid($(string(M)),vcat($(Expr(:tuple, esc.(vars)...))...))))
...
macro pcmonoid(M,args...)
    vars,exprs=looper(args)
    push!(exprs,:($(esc(M))=$GraphProductMonoid($(string(M)),vcat($(Expr(:tuple, esc.(vars)...))...))))
    return :($(foldr((x, y) -> :($x; $y), exprs, init = :()));
    );
end
...
macro subsystem(M,args...)
    ...
    for index in 1:length(args)
        ...
        k_=Symbol(string(k)*"_")
        k__=Symbol(string(k)*"__")
        push!(exprs,:(($(esc(k_)),$(esc(k__))) = $varArray($(string(k)),$(1:herm[index][2]),true)))
        push!(exprs,:($(esc(k)) = $varArray($(string(k)),$(1:herm[index][1]))))
        push!(vars,k)
        push!(vars,k_)
        push!(vars,k__)
        push!(exprs,:($(esc(systems[index]))=$NCMonoid($(string(systems[index])),vcat($(Expr(:tuple, esc.(vars)...))...))))
        push!(monoids,systems[index])
    end
    push!(exprs,:($(esc(M))=$GraphProductMonoid($(string(M)),vcat($(Expr(:tuple, esc.(monoids)...))...))))
...
macro pcmonoid_simple(M)
    exprs=[]
    push!(exprs,:(($isdefined($(esc(M)),1)) && $typeof($(esc(M))) <: GraphProductMonoid{Variable} || throw("Provide a valid Monoid")  ))
    push!(exprs,:( ($isempty($(esc(M)).vertices) || $isempty($(esc(M)).commutations)) && throw("Provide commutations")  ))
    push!(exprs,:($(esc(M))=$PCMonoid_generator($(esc(M)))))
    return :($(foldr((x, y) -> :($x; $y), exprs, init = :()));
    );
end
function Projector(var::Variable)
    isa(var.parent_monoid[],GraphProductMonoid) ? (var.mult_type[]=:Projector;var'.mult_type[]=:Projector) : (add_relations!([var*var-var]);add_relations!([var'*var'-var']))
    nothing
end
function Unipotent(var::Variable)
    isa(var.parent_monoid[],GraphProductMonoid) ? (var.mult_type[]=:Unipotent;var'.mult_type[]=:Unipotent) : (add_relations!([var*var-1]),add_relations!([var'*var'-1]))

    nothing
end
function Unitary(var::Variable)
    is_herm(var) && throw("Provide non-hermitian variable")
    isa(var.parent_monoid[],GraphProductMonoid) ? (var.mult_type[]=:Unitary;(var').mult_type[]=:Unitary) : add_relations!([var*var'-1])
