...
function cons_gen!(LMI, obj_poly, s, model, mons)
    ...
    for m in mons
        constr =
            sum((s*LMI[i][m] for i in 1:length(LMI) if haskey(LMI[i], m)), init = 0.0) -
            coefficient(obj_poly, m)
...
function npa_dual(
    ...
    if !isempty(tr_ge)
        if tracial
            ...
            tr_ge =
                [[real_rep(Polynomial(tr_ge[i][1])), tr_ge[i][2]] for i in 1:length(tr_ge)]
        end
    end
    ...
    if !isempty(tr_eq)
        if tracial
            ...
            tr_eq =
                [[real_rep(Polynomial(tr_eq[i][1])), tr_eq[i][2]] for i in 1:length(tr_eq)]
        end
    end
