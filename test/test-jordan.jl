@testset "Jordan reduction" begin
    ...
    D,C,A,b=npa_canonical(wit,level;op_ge=ge,tr_eq=eq_constr,tr_ge=ge_constr,cyclic=true,min=false,normalize=false,rm=true)
    model, P, blkD = jordan_reduce(C, A, b, verbose=true, complex=complex, diagonalize=diagonalize)
end
...
@testset "routed bell"
    ...
    diagonalize=true
    complex=true
    model_red, P, blkD = jordan_reduce(Cvec, Amat, Bvec, verbose=true,complex=complex,diagonalize=diagonalize)
end

@testset "routed bell2"
