...
function bb84_correlations(a, b, x, y; obs = false)
    ...
    proj = Dict((1, 1) => P_Z1, (1, 2) => P_Z2, (2, 1) => P_X1, (2, 2) => P_X2)
...
function chsh_correlations(a, b, x, y; obs = false)
    ...
    proj_A = Dict((1, 1) => P_Z1, (1, 2) => P_Z2, (2, 1) => P_X1, (2, 2) => P_X2)
...
model, Γ, _ =
    pcpop(0, level; min = false, optimize = false, list_vars = M.vertices, primal = true)
ηl = @variable(model)
ηs = 0.98
ηa=ηs
[
    @constraint(model, Γ[PA[a, x] * PBS[b, y]] == ηa*ηs*chsh_correlations(a, b, x, y)) for
    a in 1:2 for x in 1:2 for b in 1:2 for y in 1:2
]
id = one(A[1])
[
    @constraint(model, Γ[PA[a, x] * id] == ηa*chsh_correlations(a, -1, x, 1)) for a in 1:2
    for x in 1:2
]

[
    @constraint(model, Γ[PBS[b, y] * id] == ηs*chsh_correlations(-1, b, 1, y)) for b in 1:2
    for y in 1:2
]

[
    @constraint(model, Γ[PA[a, x] * PBL[b, y]] == ηa*ηl*chsh_correlations(a, b, x, y)) for
    a in 1:2 for x in 1:2 for b in 1:2 for y in 1:2
]

[
    @constraint(model, Γ[PBL[b, y] * id] == ηl*chsh_correlations(-1, b, 1, y)) for b in 1:2
    for y in 1:2
]
...
model, Γ, _ =
    pcpop(0, level; min = false, optimize = false, list_vars = [a; b], primal = true)
...
# AxBsy
[
    @constraint(model, Γ[a[x] * bs[y]] == ηa*ηs*chsh_correlations(1, 1, x, y; obs = true))
...
#Ax
[
    @constraint(model, Γ[a[x] * id] == ηa*chsh_correlations(1, -1, x, -1; obs = true)) for
    x in 1:2
]#2
#Ax^2
...
#Bsy
[
    @constraint(model, Γ[bs[y] * id] == ηs*chsh_correlations(-1, -1, -1, y; obs = true)) for
    y in 1:2
]#2
#Bsy^2
...
#AxBly
[
    @constraint(model, Γ[a[x] * bl[y]] == ηa*ηl*chsh_correlations(1, 1, x, y; obs = true))
    for x in 1:2 for y in 1:2
]#4
...
#Bly
[
    @constraint(model, Γ[bl[y] * id] == ηl*chsh_correlations(-1, -1, -1, y; obs = true)) for
    y in 1:2
]#2
#Bly^2
